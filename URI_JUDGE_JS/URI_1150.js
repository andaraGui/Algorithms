...
*/
const input = require('fs').readFileSync('URI_1150_stdin', 'utf8'); 
const inputLines = input.split('\n').map(Number);
let baseNumber = inputLines.shift();
let count = 1;
for (let i = 0; i < inputLines.length; i++) {
    if (inputLines[i] > baseNumber) {
        for(let x = baseNumber ; baseNumber < inputLines[i] ; x++){
            count++;
            baseNumber += x;
        }
    }
