...
*/
const input = require('fs').readFileSync('URI_1160_stdin', 'utf8');
const inputLines = input.split('\n');
const testQuant = parseInt(inputLines.shift())
for (let x = 0; x < testQuant; x++) {
    let [PA, PB, G1, G2] = inputLines.shift().split(' ').map(Number);
    let yearsCount = 0;
    while (PA <= PB) {
        yearsCount++;
